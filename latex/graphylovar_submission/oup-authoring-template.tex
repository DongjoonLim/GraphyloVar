...
\rev{A key distinguishing feature of GraphyloVar versus GPN-MSA is the explicit modeling of phylogenetic topology. Whereas GPN-MSA flattens the alignment columns into single tokens, GraphyloVar passes per-species features through a Graph Convolutional Network (GCN) whose adjacency matrix encodes the mammalian phylogenetic tree, preserving hierarchical evolutionary relationships.}
...
After transformer processing, \rev{rather than using global mean pooling, we extract the representation at the center position of each strand, the position corresponding to the variant of interest. The center features from the forward and reverse strands are concatenated into a 64-dimensional representation for each species. Extracting the center (rather than pooling over the full window) ensures that the model's downstream prediction is anchored at the variant site while still benefiting from flanking sequence context propagated via self-attention.}
...
\paragraph{\rev{GPN-MSA \citep{benegas2024gpnmsa}}} \rev{is conceptually similar to GraphyloVar in using multiple sequence alignments but differs architecturally. It processes concatenated alignment columns through transformers, which do not explicitly preserve phylogenetic relationships.}

\paragraph{\rev{GPN-Star}} \rev{GPN-Star~\citep{benegas2024gpnmsa} is an MSA-aware extension of GPN-MSA, included here as a contemporary MSA-based baseline. The large-scale genomic foundation model Evo2~\citep{nguyen2024sequence} has inference requirements (compute capability 8.9 or higher, Ada or Hopper class GPUs) that exceed our current hardware; we therefore exclude it from the quantitative benchmark and note it as an important future comparison point.}

\rev{We selected these baselines to represent diverse approaches, including conservation metrics (PhyloP, PhastCons), integrative frameworks (CADD), sequence deep learning (Enformer), and MSA-aware models (GPN-MSA, GPN-Star). While the dbNSFP database contains over 36 algorithms, we focused on these methodologically distinct families to avoid redundancy while still covering the major landscape of variant effect prediction approaches.}
...
\rev{We also analyzed MPRA studies that focused on 3'~UTR variants. \citet{Griesemer2021} conducted a genome-wide functional screen identifying causal 3'~UTR variants for human disease and evolution using GWAScatalog annotations. \citet{Schuster2023} characterized 3'~UTR variants associated with prostate cancer susceptibility. These two datasets form the 3'~UTR MPRA group in Figure~\ref{fig:MPRA}.}
\rev{For genome-wide MPRA analyses spanning diverse loci and regulatory contexts, we incorporated three additional datasets. \citet{Kircher2019} performed saturation mutagenesis of 20 disease-associated regulatory elements at single base-pair resolution. \citet{Abell2022} measured the regulatory effects of variants at multiple fine-mapped GWAS loci to resolve causal alleles underlying genetic associations in humans. \citet{McAfee2023} systematically investigated the allelic regulatory activity of schizophrenia-associated common variants across 5{,}173 fine-mapped GWAS loci. These three datasets form the genome-wide analyses group in Figure~\ref{fig:MPRA}.}
...
\begin{table}[h!]
\centering
\caption{\rev{Spearman correlation between predicted variant scores and observed minor allele frequency (MAF) on the held-out chromosomes 13-22 TOPMed variant set ($\sim$149M variants). Higher values indicate stronger alignment with population-level selective constraints.}}
\label{tab:spearman}
\begin{tabular}{lc}
\hline
...
\begin{figure}[ht!]
  ...
  \caption{Spearman correlation among variant scoring methods, calculated on a held-out TOPMed SNP dataset that was excluded from the pre-training data.}
  \label{fig:scores}
\end{figure}
...
\rev{As expected, traditional conservation metrics like PhastCons and PhyloP show stronger correlations with each other than with GraphyloVar, suggesting that GraphyloVar captures signals beyond simple nucleotide-level conservation. In the current Spearman analysis, GraphyloVar shows only weak-to-moderate correlation with major baselines such as CADD and PhyloP, which suggests that its signal is partly different from existing methods.}

\rev{To further test this, we combined GraphyloVar with CADD using a simple z-score ensemble: both scores are independently z-normalized (mean 0, standard deviation 1) over all $\sim$149M held-out variants; because CADD is oriented so that higher scores indicate greater deleteriousness (i.e., rarer variants), its z-score is negated so that the direction aligns with GraphyloVar (higher = more common); the ensemble score is then the row-wise mean of the two sign-aligned z-scores. No new network is trained---the combination is a parameter-free arithmetic operation. GraphyloVar achieves an overall held-out AUROC of $0.6246$ on the full $\sim$149M~TOPMed test set. Sign-aligned CADD achieves $\approx 0.5546$. The z-score ensemble achieves an AUROC of $0.6442$, a gain of $+0.020$ over GraphyloVar alone (Figure~\ref{fig:ensemble}). Together, the low Spearman correlations and the ensemble AUROC gain confirm that GraphyloVar captures information that CADD does not.}

\rev{To assess the statistical significance of the ensemble gains, we conducted paired two-sided DeLong tests on a uniform random sub-sample of $500{,}000$ held-out variants. The GraphyloVar~+~CADD ensemble significantly outperforms every individual baseline (vs.\ GraphyloVar alone: $\Delta$AUROC$=+0.0207$, $z=12.24$, $p<10^{-15}$; vs.\ CADD: $\Delta=+0.0950$, $z=30.62$, $p<10^{-15}$). Bootstrap percentile $95\%$ confidence intervals ($B=1000$, sub-sample $n=200{,}000$) yield a point estimate of $0.6496$ $[0.6395,\,0.6587]$ for GraphyloVar~+~CADD and $0.6271$ $[0.6174,\,0.6368]$ for GraphyloVar alone, with non-overlapping intervals; the difference between the sub-sample estimate ($0.6496$) and the full 149M-variant estimate ($0.6442$) reflects sampling variation at $n=200{,}000$ versus $n{\approx}149\text{M}$.}

\begin{figure*}[ht!]
\centering
\includegraphics[width=0.82\textwidth]{figures/fig_ensemble_auroc.png}
\caption{\rev{Held-out AUROC for individual variant scoring methods and the GraphyloVar~+~CADD z-score ensemble, evaluated on all ${\sim}$149M TOPMed variants (chromosomes~13 to 22; common vs.\ rare variant discrimination). Unlike Figure~\ref{fig:MAFBinary}A, which shows only UCSC-annotated region types, this benchmark includes the full variant set with unannotated intergenic sites, giving a $0.6246$ AUROC baseline for GraphyloVar. The ensemble is parameter-free: each score is independently z-normalized; CADD's z-score is negated to align polarity; the ensemble is the row-wise mean. Error bars show bootstrap 95\% confidence intervals ($B=1{,}000$, sub-sample $n \approx 200{,}000$). The ensemble achieves AUROC $= 0.6442$, a gain of $+0.020$ over GraphyloVar alone ($p < 10^{-15}$, DeLong test on 500{,}000 held-out variants). Non-overlapping bootstrap confidence intervals confirm that the ensemble gain is reliable and that GraphyloVar captures information that CADD does not.}}
\label{fig:ensemble}
\end{figure*}
...
As shown in Supplementary Table~\ref{tab:flank_ablation}, the region-resolved held-out AUROC values confirm that flank $= 32$ (65~bp) achieves the highest overall held-out AUROC ($0.625$) among the three settings, outperforming the narrower flank $= 16$ window ($0.622$) and the wider flank $= 100$ window ($0.617$). This result directly validates the choice of the 65~bp main model: the 65~bp window most efficiently balances local sequence context for zero-shot common-versus-rare discrimination. The fine-tuned flank $= 32$ model also demonstrates strong regulatory prediction performance, achieving the highest AUROC across all 13 MPRA benchmark datasets (Section~3.3). Exploring yet-wider windows (flank $> 100$~bp) may improve performance on tasks requiring longer-range regulatory context, and we leave this direction for future work.
...
\rev{We investigated species-level importance using an additive perturbation analysis: for each of the 58 extant placental mammal species, we replaced all other alignment rows with a gap token and measured the decrease in nucleotide cross-entropy ($\Delta$~CE) compared to an all-gap baseline on 500,000 held-out variants (chromosomes~13 to 22). This quantifies the isolated information content of each species independently of the others. As shown in Figure~\ref{fig:species_importance}, 53 of 58 species provide positive additive signal, with Human supplying the largest contribution ($\Delta$CE $= 7.1$). Among non-human species, four diverged primates form a prominent cluster: Orangutan ($\Delta$CE $= 1.50$), Green monkey ($1.50$), Gorilla ($1.24$), and Gibbon ($0.62$). Non-primate mammals show a broad range of smaller but positive contributions ($\Delta$CE $= 0.002$ to $0.12$), showing that the full multi-species alignment provides useful conservation signal beyond primates alone. Five species (Baboon, Rhesus macaque, Marmoset, Squirrel monkey, Jerboa) show near-zero or slightly negative additive values (displayed at figure floor in grey), likely because of sparse alignment coverage at these variant sites.}

\rev{Chimpanzee, despite being the closest relative of humans ($\approx$98.7\% sequence identity), ranks 11th in additive importance ($\Delta$CE $= 0.062$), well below Orangutan ($1.50$). This is explained by information redundancy: Chimpanzee's sequence is nearly identical to Human's, so the model extracts similar evolutionary signal from both. The leave-one-out marginal contribution of Chimpanzee confirms this: masking Chimpanzee while Human is present yields $\Delta$CE $\approx -2.4 \times 10^{-6}$, within statistical noise, indicating that Chimpanzee adds no incremental information on top of the Human row. In contrast, Orangutan, Green monkey, Gorilla, and Gibbon are sufficiently diverged to provide non-redundant context, explaining their higher importance in both additive and marginal analyses.}

\rev{The SE-attention gate also supports this pattern: the per-species sigmoid gate learned by the Squeeze-and-Excitation block gives higher weights to species with more useful evolutionary signal. Human ($\approx 1.00$), Orangutan ($0.54$), Green monkey ($0.40$), Gibbon ($0.34$), and Gorilla ($0.29$) have the highest gate values, while Chimpanzee has the lowest gate among great apes ($\approx 0.011$), consistent with its near-zero perturbation importance. Species with near-zero or negative additive importance (Baboon $\approx 10^{-4}$, Marmoset $\approx 2\times10^{-6}$) also show near-zero gate values. This shows that the learned weights match the perturbation importance rankings, and that the model learns to use the phylogenetic tree structure rather than treating all species equally.}


\rev{A phylogenetic tree ablation confirms that performance increases monotonically} as more species are included: restricting the model to Human alone, then adding all Primates, and finally including the full placental mammalian tree each yields consistent AUROC gains across all genomic region categories (Supplementary Figure~\ref{fig:tree_ablation}), showing that non-primate mammalian sequences also contribute useful evolutionary signal.

\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{figures/gcn_species_importance.png}
\caption{Additive species importance ($\Delta$ nucleotide cross-entropy decrease when each species is added to an all-gap baseline) for all 58 extant placental mammals, evaluated on 500,000 held-out variants (chromosomes~13 to 22). Species are sorted by decreasing importance; bars are colored by mammalian clade (see legend). The y-axis is logarithmic. Five species with near-zero or marginally negative additive values (Baboon, Rhesus macaque, Marmoset, Squirrel monkey, Jerboa) are displayed at the figure floor in grey. Human provides the largest isolated signal ($\Delta$CE $= 7.1$). Among non-human species, four diverged primates, Orangutan ($1.50$), Green monkey ($1.50$), Gorilla ($1.24$), and Gibbon ($0.62$), form a clearly elevated cluster, followed by a broad band of non-primate mammals ($0.002$ to $0.12$). Chimpanzee ranks 11th ($\Delta$CE $= 0.062$), consistent with information redundancy: its near-identical sequence ($\approx$98.7\% identity) provides little additive signal beyond what Human already encodes.}
...
Despite these advantages, some limitations remain. The reliance on multiple sequence alignments requires extensive computational time compared to single-sequence methods. Additionally, the current implementation focuses on substitutions rather than larger structural variants. Future work could address these constraints by exploring more efficient alignment processing, direct modeling of structural variants, integration with functional genomic data to capture tissue-specific effects, and wider context windows (flank $> 100$~bp) that may better capture long-range regulatory interactions in enhancer and promoter contexts.
...
GraphyloVar source code, trained model weights (flank=32 checkpoint), and preprocessing scripts are available at \url{https://github.com/DongjoonLim/GraphyloVar}. The TOPMed whole-genome sequencing data used for pre-training are available through dbGaP (accession phs000964). UCSC 100-way vertebrate whole-genome alignments are publicly available at \url{https://hgdownload.soe.ucsc.edu/goldenPath/hg38/multiz100way/}. MPRA datasets used for evaluation are available from the original publications cited in Section~2.5.
...
\centering
\small
\caption{\rev{Region-resolved held-out AUROC for three context-window sizes, evaluated on the full ${\sim}$149M TOPMed SNVs from chromosomes~13 to 22. The main benchmark model (flank $= 32$, 65~bp) is indicated by $\dagger$. Bold values indicate the highest AUROC in each column.}}
\label{tab:flank_ablation}
\begin{tabular}{rrrrrrr}
\hline
\textbf{Flank} & \textbf{Input (bp)} & \textbf{All} & \textbf{Coding} & \textbf{3'~UTR} & \textbf{cCREs} & \textbf{TE} \\
\hline
16  & 33  & 0.622 & \textbf{0.625} & 0.619 & \textbf{0.620} & 0.621 \\
$32^{\dagger}$ & 65 & \textbf{0.625} & 0.616 & \textbf{0.621} & 0.617 & \textbf{0.626} \\
