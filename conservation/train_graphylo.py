...
from tensorflow.keras.layers import Dense, Input, Dropout, LSTM, MaxPooling2D, MaxPooling1D
...
X_train = np.load('graphs/{}/X_revCompConcatenatedTrue_{}.npy'.format(transcriptionFactor,celltype))
...
def model_conv2d(X_train, hidden):
    tf.keras.backend.clear_session()
    inputs = Input(shape=(X_train.shape[1:]), dtype='uint8')
    x = Embedding(1000, 6, input_length=201)(inputs)
    x = Conv2D(32, 7, padding='same', activation='relu', input_shape=x.shape[1:])(x)
    x = Dropout(.3)(x)
    ...
    x = GCNConv(64, activation="relu", kernel_regularizer=l2(5e-4))([x,A])
...
def model_conv1d(X_train, hidden):
    tf.keras.backend.clear_session()
    inputs = Input(shape=(X_train.shape[1:]), dtype='uint8')
    x = Embedding(1000, 6, input_length=201)(inputs)
    
    x = Conv1D(32, 7, padding='same', activation='relu', input_shape=x.shape[2:])(x)
    x = Dropout(.3)(x)
...
#     x = Conv1D(64, 11, padding='same', activation='relu', input_shape=x.shape[2:])(x)
#     x = Dropout(.3)(x)
#     x = Conv1D(32, 7, padding='same', activation='relu')(x)
...
def model_conv1d_test(X_train, hidden):
    tf.keras.backend.clear_session()
    inputs = Input(shape=(X_train.shape[1:]), dtype='uint8')
    ...
    x = Dropout(.3)(x)
    x = Conv1D(32, 10, padding='same', activation='relu')(x)
...
def model_conv2d_test(X_train, hidden):
    tf.keras.backend.clear_session()
    inputs = Input(shape=(X_train.shape[1:]), dtype='uint8')
    x = tf.one_hot(inputs, 6)
    print(x.shape)
    x = Conv2D(115, (10,6), padding = 'same', activation='relu', data_format='channels_first', input_shape=x.shape[1:])(x)
    x = Dropout(.3)(x)
    print(x.shape)
...
def modelFit(filepath, epoch, batchSize, X_train, y_train, model):
...
for hidden in hidden_sizes:
    print(hidden)
    model_conv2d = model_conv2d_test(X_train, hidden)#.reshape(-1, 113, 15, 1))
    # model_conv1d = model_conv1d_test(X_train, hidden)#.reshape(-1, 113, 15, 1))
    
    # hist_conv2d,model15_conv2d = modelFit('kerasModels/model{}_conv2d_{}_{}_hidden{}'.format(201,transcriptionFactor, celltype, hidden), 9000, 256, X_train, y_train, model_conv2d)
    # hist_conv1d,model15_conv1d = modelFit('kerasModels/model{}_conv1d_{}_{}_hidden{}'.format(201,transcriptionFactor, celltype, hidden), 9000, 256, X_train, y_train, model_conv1d)

    hist_conv2d,model15_conv2d = modelFit('kerasModels/model{}_conv2d_{}_{}_hidden{}_revCompConcatenated'.format(201,transcriptionFactor, celltype, hidden), 9000, 256, X_train, y_train, model_conv2d)
...
print(X_train_onehot.shape)
model = model_onehot(X_train_onehot)
hist, model = modelFit('kerasModels/model{}_conv2d_{}_{}_hidden{}_revCompConcatenated_onehot'.format(201,transcriptionFactor, celltype, 64), 9000, 256, X_train_onehot, y_train, model)
