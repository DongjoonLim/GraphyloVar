...
def model1D_nograph_onehot_se(X_train_shape):
    tf.keras.backend.clear_session()
    length = int(X_train_shape[2]/2)
    input = Input(shape=(X_train_shape[1:]), dtype='uint8')
    x = tf.one_hot(input, 6)
    ...
    x = Conv1D(32, 11, padding = 'valid', data_format='channels_first', activation='relu', input_shape=x.shape[1:])(x)
    x = species_attention(x)
    x = Dropout(.3)(x)
    x = MaxPooling2D((2, 1), data_format='channels_first')(x)
    print(x.shape)
...
y_train = np.load('graphs/{}/y_revCompConcatenated_{}.npy'.format(transcriptionFactor,celltype))
np.random.seed(1234)
shuffle_ind = np.random.permutation(X_train.shape[1])
X_train = X_train[:, shuffle_ind, :]
names_shuffled = [names[x] for x in shuffle_ind] 
...
for hidden in hidden_sizes:
    print(hidden)
    # model= model1D_siamese_se(X_train.shape)
    # model= model1D_siamese_se_linear(X_train.shape)
    # model= model1D_nograph_onehot_se(X_train.shape)
    model= model1D_siamese_se(X_train.shape)


#    hist, model = modelFit('kerasModels/model{}_conv1d_siamese_{}_{}_hidden{}_revCompConcatenated_onehot_se'.format(201,transcriptionFactor, celltype, 64), 9000, 64, X_train_onehot, y_train, X_val_onehot, y_val, model)
    # hist, model = modelFit('kerasModels/model{}_conv1d_siamese_{}_{}_hidden{}_revCompConcatenated_se_randomizedTree'.format(201,transcriptionFactor, celltype, 64), 9000, 64, X_train, y_train, X_val, y_val, model)
    hist, model = modelFit('kerasModels/model{}_conv1d_siamese_{}_{}_hidden{}_revCompConcatenated_se_focalloss'.format(201,transcriptionFactor, celltype, 64), 9000, 64, X_train, y_train, X_val, y_val, model)
