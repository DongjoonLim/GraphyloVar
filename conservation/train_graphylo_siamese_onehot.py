...
from tensorflow.keras.layers import Dense, Input, Dropout, LSTM, MaxPooling2D, MaxPooling1D, MaxPooling3D
...
from tensorflow.keras.layers import Attention, Dense, Input, Dropout, LSTM, Flatten,  Embedding, Attention, Reshape, Bidirectional, Conv1D, Conv2D, AdditiveAttention
from tensorflow.keras.models import Model
from tensorflow.keras.optimizers import Adam
...
def model_siamese_onehot(X_train):
    tf.keras.backend.clear_session()
    length = int(X_train.shape[2]/2)
...
def model_siamese1D_onehot(X_train):
    tf.keras.backend.clear_session()
    length = int(X_train.shape[2]/2)
    inputs = Input(shape=(X_train.shape[1:]))
    inputs_l, inputs_r = tf.split(inputs, [length,length], 2)
...
def model_conv3d_siamese(X_train):
    tf.keras.backend.clear_session()
    length = int(X_train.shape[2]/2)
    inputs = Input(shape=(X_train.shape[1:]), dtype='uint8')
    x = tf.one_hot(inputs, 6)
...
def model_conv3d_bahdanau(X_train):
    length = int(X_train.shape[2]/2)
    inputs = Input(shape=(X_train.shape[1:]))
    inputs_l, inputs_r = tf.split(inputs, [length,length], 2)
...
# X_train, X_val, X_train_onehot, X_val_onehot, y_train, y_val = train_test_split(X_train,X_train_onehot, y_train, test_size=0.4, random_state=42)
# X_val, X_test, X_val_onehot, X_test_onehot, y_val, y_test = train_test_split(X_val, X_val_onehot , y_val, test_size=0.5, random_state=42)
...
for hidden in hidden_sizes:
    print(hidden)
    model_conv3d_attention = model_conv3d_bahdanau(X_train_onehot)#.reshape(-1, 113, 15, 1))
    # model_conv1d = model_conv1d_test(X_train, hidden)#.reshape(-1, 113, 15, 1))
    
    hist_conv3d_attention, model_conv3d_attention = modelFit('kerasModels/model{}_conv3d_bahdanau_{}_{}_hidden{}_onehot'.format(201,transcriptionFactor, celltype, hidden), 9000, 256, X_train_onehot, y_train, model_conv3d_attention)
