...
def binaryTarget(val1, val2, functionality):
    ...
    if np.array_equal(val1, val2) and functionality: # positive example is conserved
        return 1 # np.array([0, 1])
    elif  (not np.array_equal(val1, val2)) and (not functionality) : #np.array([1, 0]) # negative example is mutated
        return 0
    else :
        return -1
...
positive = positive.loc[positive['y'] == 1].sample(negative.shape[0],random_state=42)
...
names = ['gorGor3', 'ponAbe2', 'nomLeu3', 'rheMac3', 'macFas5', 'papAnu2', 'chlSab2', 'calJac3', 'saiBol1', 'otoGar3', 'tupChi1', 
...
#     G.add_node(i, name=i)
#edges
edges = [('gorGor3','_HPG'),('ponAbe2','_HPGP'),('_HP','_HPG'), ('_HPG','_HPGP'), ('nomLeu3', '_HPGPN'), 
...
toPickle('graphs/train_labels_chr{}_ccre.pkl'.format('All'), list(seqdf['y']))
toPickle('graphs/train_indices_chr{}_ccre.pkl'.format('All'), list(seqdf.index))
dataset_15 = ModDataset(seqdf, rawdf, A, 7, transform =NormalizeAdj())
toPickle('graphs/dataset_15_chr{}_ccre.pkl'.format('All'), dataset_15)

dataset_51 = ModDataset(seqdf, rawdf, A, 25, transform =NormalizeAdj())
toPickle('graphs/dataset_51_chr{}_ccre.pkl'.format('All'), dataset_51)

dataset_101 = ModDataset(seqdf, rawdf, A, 50, transform =NormalizeAdj())
toPickle('graphs/dataset_101_chr{}_ccre.pkl'.format('All'), dataset_101)
