...
# In[2]:
#val 19, test 20, train other
celltype = sys.argv[1]
chromosomes = [2,3,4,5,6,7,9,10,11,12,16,17,18] #,4,5,6,7,8,9,10]
seqDictRaw = [] 
...
ccres = []
non_ccres = []
tf =  pd.read_csv('encode/chipseq/labels/tsvs/CTCF.train.labels.tsv.gz', delimiter = '\t')
ccre = tf.loc[tf[celltype]=='B']
non_ccre = tf.loc[tf[celltype]=='U']
for chromosome in chromosomes :
    ccre = ccre.loc[ccre['chr'] == 'chr{}'.format(chromosome)]
    ccres.append(ccre)
    non_ccre = non_ccre.loc[non_ccre['chr'] == 'chr{}'.format(chromosome)]
    non_ccres.append(non_ccre)
...
def getFunctionality(ccres, non_ccres):
    functionalities = []
    for i in range (len(ccres)):
        starts = list(ccres[i]['start'].astype(int))
        stops = list(ccres[i]['stop'].astype(int))
        starts_non = list(non_ccres[i]['start'].astype(int))
        stops_non = list(non_ccres[i]['stop'].astype(int))
        length = len(seqDictRaw[i]['hg38'])
        functionality = [-1]*length
        for start, stop in zip(starts, stops):
            if stop < length:
                functionality[int((start+stop)/2)] = 1
            else :
                break
        for start_non, stop_non in zip(starts_non, stops_non):
            if stop_non < length:
                functionality[int((start_non+stop_non)/2)] = 0
            else :
                break
...
functionalities = getFunctionality(ccres, non_ccres)
...
# # np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_X_test.npy'.format('All', celltype), X_train101)
# # np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_y_test.npy'.format('All', celltype), y_train101)
...
dataset_201 = ModDataset(seqdf, rawdf, A, 100, transform =NormalizeAdj())
# toPickle('graphs/dataset_101_chr{}_ccre_nonccre.pkl'.format('All'), dataset_101)
X_train201, y_train201 = preprocess(dataset_201)
# np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_X_train.npy'.format('All', celltype), X_train101)
# np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_y_train.npy'.format('All', celltype), y_train101)
# np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_X_test.npy'.format('All', celltype), X_train101)
# np.save('graphs/dataset_101_chr{}_ccre_nonccre_{}_y_test.npy'.format('All', celltype), y_train101)
np.save('graphs/dataset_201_chr{}_CTCF_{}_X_train.npy'.format('All', celltype), X_train201)
np.save('graphs/dataset_201_chr{}_CTCF_{}_y_train.npy'.format('All', celltype), y_train201)
